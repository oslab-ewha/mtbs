...
__device__ unsigned
cu_get_tb_sm_rr(unsigned n_mtbs, unsigned *pidx_mtb_start)
{
	...
	do {
		...
		if (mtb_start > 0) {
			...
			if (id_sm == 0)
				id_sm = d_fkinfo->n_sm_count;
			*pidx_mtb_start = mtb_start;
			return id_sm_cur;
		}
		id_sm_cur = (id_sm_cur + 1) % d_fkinfo->n_sm_count;
		if (id_sm_cur == 0)
			id_sm_cur = d_fkinfo->n_sm_count;
	}

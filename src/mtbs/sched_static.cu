...
#define mAO_EPOCH_HOST(cinfo, epoch, id_sm, idx)	(mTB_ALLOCOFF_TABLE_EPOCH_HOST(cinfo, epoch) + mTB_INDEX_HOST(id_sm, idx) - 1)
...
static void
apply_mAOT_uprange(unsigned char epoch, unsigned id_sm, unsigned idx, skrid_t skrid, unsigned short offset)
{
	...
	mAO = mAO_EPOCH_HOST(cinfo, epoch, id_sm, idx);
	mAO->skrid = skrid;
	mAO->offset = offset;
...
set_mtbs_skrid(sched_ctx_t *pctx, unsigned id_sm, unsigned short offbase, unsigned n_mtbs, unsigned char *epochs)
{
	...
	for (i = 1; i <= n_max_mtbs_per_sm; i++) {
		...
		apply_mAOT_uprange(epoch, id_sm, i, pctx->skrid, offbase + off);
		apply_mAOT_uprange(NEXT_EPOCH(epoch), id_sm, i, 0, (unsigned short)-1);

...
static void
set_mtbs_skrid(sched_ctx_t *pctx, unsigned idx_sm, unsigned idx_MTB, unsigned n_mtbs, unsigned char barid, unsigned char *epochs)
{
	unsigned	n_mtbs_cur = 0;
	...
	for (i = 1; i <= n_mtbs_per_MTB; i++) {
		...
		apply_mAOT_uprange(epoch, idx_sm, idx_MTB, i, pctx->skrid, offbase, barid);
...
assign_tb_by_rr(sched_ctx_t *pctx, unsigned n_mtbs)
{
	...
	for (i = 0; i < n_sm_count; i++) {
		...
		if (find_mtbs_on_sm(idx_sm, idx_MTB, n_mtbs, pctx->epochs)) {
			...
			set_mtbs_skrid(pctx, idx_sm, idx_MTB, n_mtbs, barid, pctx->epochs);
